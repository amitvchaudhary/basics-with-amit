...
It is the _MyPhoenixApp_ module which is reffered when you run your application through **iex -S mix** or **mix phx.server** command. Below _start_ function, inside _MyPhoenixApp_ module is automatically defined you create the app and invoked when the app is started.
...
**conn** is nothing but a simple struct with lots of keys to store both request and response unlike other frameworks where both are stored separately.
...
2. From the request data, the **Handler** creates the most awaiting topic of our post, the **conn** struct
