...
As we have not added `after` clause in the process, it will keep waiting for new messages. So try from your end to send few more messages. How do I know if it's waiting? You can use `Process.info/0` to get all the details of the running process.
...
If you just want to see specific fields of interest, you can use `Process.info/2` where you can send the list of fields as second argument.

Continue from https://www.openmymind.net/Elixir-A-Little-Beyond-The-Basics-Part-6-processes/
