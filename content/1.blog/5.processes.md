...
Above code will accept messages only if they are in `{from, operation, num1, num2}` pattern. All other messages will remain in the mailbox forever occupying memory for no reason. Now this same can also cause performance slowdown. Recall how pattern matching works. The process picks up one message, matches against all patterns and puts it back if it doesn't match. If new message comes, it will start from the begining, reading old messages first and then the newest one. So, if you have 1000 messages in queue, when 1001 message arrives, the process will first process 1000 messages and then the 1001 one. This way process wastes time iterating through unhandled messages and its throughput suffers.
