...
In short, when you use `is_even`, other developers will know that this is a guard and not a function. G

- Use `!` at the end of the name if the function is going to raise an exception. You will find this pattern more in core and third-party library functions. This is because not every function will execute successfully if you provide incorrect input.
...
One question you may have is why did we create separate module? Because, as per the official docs, _functions defined in the same module as the attribute itself cannot be called because they have not yet been compiled when the attribute is being defined._
...
As you can see, just pattern matching won't work because variables `a` and `b` can accept anything. Here, we can use guard to protect our function and check whether the arguments passed are numbers and if yes then only execute our function.
...
```elixir
iex(12)> OurMathModule.area({:rectangle, "10", "20"})
...
Because we don't have any default clause it threw above error. Let's add one.
